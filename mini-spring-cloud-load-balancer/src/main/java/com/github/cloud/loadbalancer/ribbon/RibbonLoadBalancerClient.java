...
public class RibbonLoadBalancerClient implements LoadBalancerClient {
    ...
    public <T> ServiceInstance choose(String serviceId, Request<T> request) {
        //获取到serviceId对应的ribbon 组件 （一些策略）
        ILoadBalancer loadBalancer = springClientFactory.getInstance(serviceId, ILoadBalancer.class);
     ...
     */
    @Override
    public URI reconstructURI(ServiceInstance serverInstance, URI original) {
        try {
            //将服务名称替换成服务实例的IP:端口 例如http://provider-application/echo 被替换成 http://192.168.100.1:8888/echo
            StringBuilder sb = new StringBuilder();
            sb.append(original.getScheme()).append("://");
            sb.append(serverInstance.getHost());
            sb.append(":").append(serverInstance.getPort());
